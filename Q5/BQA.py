...
dataset = load_dataset('boolq', split='validation[:100]')
...
item = dataset[27] # Getting a case where the api dropped out
print(prompt_base + '\n\nQ: ' + item['question'] + '\nP: ' + item['passage'] + "\nA: ")
output = query({
	"inputs": prompt_base + '\n\nQ: ' + item['question'] + '\nP: ' + item['passage'] + "\nA: ",
})

print(output[0]['generated_text'].split(prompt_base + '\n\nQ: ' + item['question'] + '\nP: ' + item['passage'] + "\nA: "))

golds = []
preds = []

for item in dataset[27]:
    ...
    if type(output) is not dict:
        preds.append(output[0]['generated_text'].split(prompt_base + '\n\nQ: ' + item['question'] + '\nP: ' + item['passage'] + "\nA: ")[1])
        print(output[0]['generated_text'].split(prompt_base + '\n\nQ: ' + item['question'] + '\nP: ' + item['passage'] + "\nA: ")[1])
    else:
        preds.append('pass')
        print(output)
correct = 0
f_resps = open("pred_body.txt", 'w')
with open("model_preds.txt", 'w') as f:
    f.write('pred|gold\n')
    for pred, gold in zip(preds, golds):
        if ('true' in pred) or ('false' in pred) or ('no' in pred):
        ...
        f_resps.write(pred + '\n')
print(f'BLOOMZ Accuracy: {correct} of {len(golds)} ({(correct / len(golds)) * 100}%)')
